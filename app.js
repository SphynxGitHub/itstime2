...
// 2. Request Permission & Subscribe User
async function subscribeToPushNotifications() {
  ...
  // Public VAPID Key generated from web-push library
  ...
  // Save device subscription to your Supabase patient/user record
  const user = (await supabaseClient.auth.getUser()).data.user;
  await supabaseClient
    .from('push_subscriptions')
    .upsert({ user_id: user.id, subscription: JSON.stringify(subscription) });
